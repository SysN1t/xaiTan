...
public class XiaTan implements BurpExtension, HttpHandler, ContextMenuItemsProvider {
    ...
    public void initialize(MontoyaApi api) {
        this.api = api;
        api.extension().setName("XiaTan v2.1.2 (Montoya)");

        scanEngine = new ScanEngine(api);

        SwingUtilities.invokeLater(() -> {
            panel = new XiaTanPanel(api, scanEngine, this);
            api.userInterface().registerSuiteTab("XiaTan", panel);
        });

        api.http().registerHttpHandler(this);
        api.userInterface().registerContextMenuItemsProvider(this);

        // 扩展卸载时清理资源（Swing Timer / 线程池等）
        ...
        api.logging().logToOutput("  XiaTan v2.1.2 loaded! (Montoya API)");
        api.logging().logToOutput("  Author: SysN3t");
        api.logging().logToOutput("  Probe: XSS / SQLi / SSTI / NoSQLi");
        api.logging().logToOutput("  Right-click -> Send to XiaTan");
        api.logging().logToOutput("==============================================");
    }
    ...
    public List<Component> provideMenuItems(ContextMenuEvent event) {
        ...
        JMenuItem scanAll = new JMenuItem("发送到 XiaTan");
        ...
        JMenu sub = new JMenu("XiaTan 选择性扫描...");
